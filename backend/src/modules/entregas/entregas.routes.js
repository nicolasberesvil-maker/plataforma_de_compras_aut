...
router.patch('/:id/en-transito', requireRole(['ADMIN']), validate(marcarEnTransitoSchema), ctrl.marcarEnTransito);
...
router.patch('/:id/confirmar-entrega-campo', requireRole(['ADMIN', 'PRODUCTOR']), validate(confirmarEntregaCampoSchema), ctrl.confirmarEntregaCampo);
router.patch('/:id/cancelar', requireRole(['ADMIN']), validate(cancelarEntregaSchema), ctrl.cancelar);
