...
router.get('/', requireRole(['ADMIN', 'OPERADOR']), validate(filtrosIntencionSchema, 'query'), ctrl.listar);
router.post('/', requireRole(['PRODUCTOR']), validate(crearIntencionSchema), ctrl.crear);
router.post('/agrupar', requireRole(['ADMIN', 'OPERADOR']), validate(agruparIntencionesSchema), ctrl.agrupar);

router.get('/:id', requireRole(['PRODUCTOR', 'ADMIN', 'OPERADOR']), ctrl.obtenerPorId);
...
router.post('/:id/descartar', requireRole(['ADMIN', 'OPERADOR']), validate(descartarIntencionSchema), ctrl.descartar);
